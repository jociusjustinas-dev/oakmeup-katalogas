...
/** Lexical `let`/`const` catalog globals are not on `window`; Framer script reads `window.*`. */
...
.catalog-root--grid .prod-card:hover{transform:translateY(-2px)!important;background:#1C3A13!important}.catalog-root--grid .prod-card.selected{box-shadow:0 0 0 2px #D3FA99 inset!important;background:#1C3A13!important}.catalog-root--grid .prod-card::after{content:none!important}
...
(function(){
    ...
    function installRenderPostHeight(){
        if (typeof window.render !== 'function' || window.__omuRenderHeightHook) return;
        window.__omuRenderHeightHook = true;
        var r0 = window.render;
        window.render = function(){
            var out = r0.apply(this, arguments);
            try { setTimeout(postHeight, 0); } catch (e0) {}
            return out;
        };
    }
    ...
    function moveActions(){
        patchSubmitHandlers();
        installRenderPostHeight();
        document.querySelectorAll('.catalog-root--grid .prod-card').forEach(function(card){
            var sku = getInquirySkuFromCard(card);
            if (!sku) return;
            ...
            if (addInq) {
                ...
                addInq.textContent = selOn ? 'Išimti iš pasiūlymo' : 'Pridėti į pasiūlymą';
    ...
    return documentHtml.replace(/<\/body>/i, `${actionSwapScript}</body>`)
}
